...
//    var zoomOutGestureRecognizer = UISwipeGestureRecognizer()
    ...
    static var picture: UIImage = UIImage(named: "noPhoto")!
    static var acquired: Bool = false
    ...
    func CreateUI(){
       ...
       // self.cameraPreviewLayer?.connection?.videoOrientation = .portraitUpsideDown
        ...
        self.cameraPreviewLayer?.frame = CGRect(x: 0, y: 0, width: WIDTH, height: HEIGHT)
    ...
    func createPhotoBtn(){
        //创建缩放
        self.zoomSlider = UISlider.init(frame: CGRect.init(x: WIDTH/2 - 150, y: HEIGHT - 150, width: 300, height: 50))
        ...
        //创建照相按钮
        self.photoBtn = UIButton.init(frame: CGRect.init(x: WIDTH/2 - 50, y: HEIGHT - 220, width: 100, height: 80))
        ...
        //创建取消按钮
        self.cancel = UIButton.init(frame: CGRect.init(x: 50, y: HEIGHT - 220, width: 100, height: 80))
       ...
       self.cancel.addTarget(self, action: #selector(self.cancelAction), for: .touchUpInside)
        ...
        //创建保存按钮
        self.save = UIButton.init(frame: CGRect.init(x:WIDTH - 150, y: HEIGHT - 220, width: 100, height: 80))
 ...
 @objc func saveAction(){
     ...
     camera.picture = self.photoImageview.image!
     camera.acquired = true
     util.getPoints(position: 960)
     util.LoadToRgbdata(data: RGB_DATA)
     self.dismiss(animated: true, completion: nil)
     }
 }

extension camera: UITextFieldDelegate,AVCapturePhotoCaptureDelegate{
